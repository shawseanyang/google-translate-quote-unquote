...
# large numbers may take a while or be blocked by Google Translate due to script detection
...
def block_check(gtranslate_result):
    """Check if Google Translate blocked translation request."""
...
def main():
    ...
    Takes INPUT and translate it LAYERS times through a randomly chosen slew of languages
    """
    ...
    for i in range(LAYERS):
        ...
        # pick another random language if a language is consecutively repeated
        ...
        # converts special characters to their URL-safe, %-escaped versions
        buffer = quote(buffer)
        ...
        if (i%3 == 0):
            # sleep every 3rd query so Google Translate doesn't block this script
            time.sleep(1)

    # after slew of languages, translate back to input language
